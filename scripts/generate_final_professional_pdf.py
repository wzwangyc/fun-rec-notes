...
# 创建 PDF 文档
output_path = r"C:\Users\28916\.openclaw\workspace\scripts\中兵红箭投资价值分析报告_最终专业版.pdf"
...
# ========== 颜色定义 ==========
primary_color = HexColor('#1e3a8a')  # 深蓝
secondary_color = HexColor('#374151')  # 深灰
accent_color = HexColor('#ef4444')  # 红色
light_bg = HexColor('#eff6ff')  # 浅蓝背景
risk_bg = HexColor('#fef2f2')  # 浅红背景
border_color = HexColor('#3b82f6')  # 边框蓝
...
cover_english_style = ParagraphStyle(
    'CoverEnglish',
...
# 顶部标识
content.append(Paragraph("EQUITY RESEARCH", cover_subtitle_style))
content.append(Spacer(1, 0.5*cm))

# 主标题
content.append(Paragraph("中兵红箭 (000519.SZ)", cover_title_style))
content.append(Paragraph("投资价值分析报告", cover_subtitle_style))
content.append(Paragraph("Equity Research Report", cover_english_style))
...
rating_table = Table(rating_data, colWidths=[4.5*cm, 7*cm])
...
# 分析师信息
analyst_info = Paragraph(f"编制：Leo 智能投研\n报告日期：{datetime.now().strftime('%Y 年 %m 月 %d 日')}", normal_style)
content.append(analyst_info)

content.append(PageBreak())

# ========== 免责声明 ==========
content.append(Paragraph("重要声明", section_style))
disclaimer = """
• 本报告由 Leo 智能投研制作，仅供客户参考
• 报告中的信息均来源于公开资料，本公司对这些信息的准确性、完整性或可靠性不作任何保证
• 本报告所载资料、意见及推测仅反映本公司于发布本报告当日的判断
...
# ========== 一、执行摘要 ==========
content.append(Paragraph("一、执行摘要", section_style))
...
# ========== 二、公司概况 ==========
content.append(Paragraph("二、公司概况", section_style))
...
# ========== 三、行业分析 ==========
content.append(Paragraph("三、行业分析", section_style))
...
# ========== 四、财务分析 ==========
content.append(Paragraph("四、财务分析", section_style))
...
# ========== 五、估值分析 ==========
content.append(Paragraph("五、估值分析", section_style))
...
# ========== 六、风险因素 ==========
content.append(Paragraph("六、风险因素", section_style))

risk_box_data = [['风险提示']]
risk_box = Table(risk_box_data, colWidths=[15*cm])
...
# ========== 七、投资建议 ==========
content.append(Paragraph("七、投资建议", section_style))
...
# ========== 免责声明 ==========
content.append(Paragraph("免责声明", section_style))
disclaimer_text = """
